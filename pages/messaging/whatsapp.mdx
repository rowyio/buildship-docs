...
<Callout emoji='💡'>
  This node is only to send a single free-form WhatsApp Message. Use the WhatsApp Bot for a complete streamlined
  automation process [here](https://docs.buildship.com/trigger-nodes/whatsapp-bot).
</Callout>
...
### 📝 Steps to get required credentials and permissions from Meta - WhatsApp Business Account
...
<Callout emoji='💡'>
  Note that you have a temporary access token. For security reasons, the token expires in less than 24 hours. To get a
  permanent access refer [here](#steps-to-get-a-permanent-access-token-never-expires).
</Callout>
...
### Time to Test and Ship WhatsApp Node 🚀
...
<Callout emoji='💡' type='tip'>
  Use the [WhatsApp Bot Trigger](/trigger-nodes/whatsapp-bot) to automate the process of sending messages to your
  customers.
</Callout>
