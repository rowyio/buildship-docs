...
If there’s no matching constant specified in the "case" inputs, control is transferred to the flow under the
"**default**" case.
...
which is a fallback case - i.e. if there’s no matching constant specified in the "case" inputs, control is transferred
to the flow under the "default" case. For the Default case, we can simply add a return node saying that the `type` is
undefined.
