...
Once we have built and tested the workflow, we can link ways to initiate this workflow. We can do this under the “Connect” tab.
...
- Hit **Connect**.
- Once your trigger has successfully been connected, you'll be able to access addition settings to configure your workflow inputs and outputs.
...
The **Workflow inputs schema** option lists all of your workflow inputs (required/optional). The Inputs for your workflow have already been defined in the **Output Node**. 
...
Imagine you have a trigger set up, but you're not sure what the incoming request looks like. We can make use of the Get Data feature in the post-setup preview screen.
...
Once a request is found delivered to the endpoint, you'll be **displayed a tree structure of the properties received** in the request. **Select the required inputs for your workflow and click on “Add inputs to schema”**. This would automatically add the selected properties as inputs to your workflow auto-mapped.
...
<Cards num={3}>
  ...
  <Card image arrow title='Every Month (Cron) Trigger' href='/trigger-nodes/every-month'>
    <>![Every Month (Cron) Trigger](/assets/every-month.png)</>
  </Card>
  <Card image arrow title='Every Week (Cron) Trigger' href='/trigger-nodes/every-week'>
    <>![Every Week (Cron) Trigger](/assets/every-week.png)</>
  </Card>
