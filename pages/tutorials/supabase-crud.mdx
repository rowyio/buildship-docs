...
To perform CRUD operations on your Supabase database, you'll require to create a custom RLS (Row Level Security), as by default RLS is enabled with a policy to restrict all access using `anon key` which will prevent you to perform operation and will result in empty array.
...
BuildShip's secret manager, learn more [here](https://docs.buildship.com/workflow-operations).
...
<Callout emoji="💡" >
There are two nodes, one with `Token` and without `Token`. The node with `Token` uses your Supabase User Access Token from the client. This allows Supabase Row Level Security to be utilized as a passthrough with Buildship.
...
Note: If you do not have an Access Token, use the Supabase "Get Row" node instead.
...
Note: If you do not have an Access Token, use the Supabase "Update Row" node instead.
...
Note: If you do not have an Access Token, use the Supabase "Delete Row" node instead.
